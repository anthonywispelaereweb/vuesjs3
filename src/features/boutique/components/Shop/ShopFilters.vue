...
<template>
  <div class="p-20 d-flex flex-column">
    <section class="mb-20">
      ...
      <input :value="filters.search" @input="emit('updateFilter', { search: ($event.target as HTMLInputElement).value })"
    ...
    <section class="mb-20 flex-fill">
      ...
      <p class="category" :class="{ selected: filters.category === category }"
        v-for="(category, i) in (['all', 'desktop', 'gamer', 'streaming'] as Category[])" 
        :key="i"
